...
| Block behaviors (`behavior` field) | 43 kinds | solid, plant (support), log, ore, leaves (decay), sapling (growth), slab, stairs, torch (support), glass, pane, fence, wall, carpet, snow_layer (stack/melt), container, furnaces, crafting table, falling, fluid (flow), crop (growth), spreading, farmland, door, trapdoor, fence_gate, bed, button, lever, climbable, ice (melt), growing (cane/cactus/bamboo), spawner (mob batches with vanilla's timing and caps) | workstation (crafting and enchanting tables, anvils, grindstone, stonecutter, smithing table; loom, cartography table, beacon, lectern and crafter only show a toast), pressure_plate (support only), decoration | redstone, sign, banner, head, candle, coral, fire, portal, unbreakable |
...
| Loot tables | 1085 blocks, 158 entities, 56 chests, gameplay/archaeology | block drops (silk touch, fortune, block-state conditions), entity drops (killed-by-player, looting, cooked when burning) and chest tables (structure chests roll their vanilla table, enchanted gear included) | – | gameplay/archaeology tables unused |
